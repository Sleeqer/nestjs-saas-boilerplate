 ...
 * Queue
 */
export const _QUEUE: string = `${config.get('APPLICATION_NAME')}_users`;
...
const handler = async (
  ...
  try {
    ...
    if (logger)
      logger.error(`[UserHandler] -> Error on handler (${error.message})`);
...
export class UserHandler extends MessageHandler {
  /**
   * Constructor of User Handler Class
