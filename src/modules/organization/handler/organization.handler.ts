 ...
 * Queue
 */
export const _QUEUE: string = `${config.get('APPLICATION_NAME')}_organizations`;
...
const handler = async (
  ...
  try {
    ...
    if (logger)
      logger.error(
        `[OrganizationHandler] -> Error on handler (${error.message})`,
      );
...
/**
 * Organization Handler Class
 */
export class OrganizationHandler extends MessageHandler {
  /**
   * Constructor of Organization Handler Class
 ...
 */
export default [
  new OrganizationHandler(handler, {
    exchange: {
