...
import { EXCHANGE, KEY, QUEUE } from './profile.handler.enum';
 ...
 * Queue
 */
export const _QUEUE: string = `${config.get('APPLICATION_NAME')}_profiles`;
...
const handler = async (
  ...
  try {
    ...
    if (logger)
      logger.error(`[ProfileHandler] -> Error on handler (${error.message})`);
 ...
 */
export class ProfileHandler extends MessageHandler {
  /**
   * Constructor of Profile Handler Class
