 ...
 */
import { MessageHandler as Handler } from '../../../adapters/rabbitmq/handler/message.handler';
import { RabbitMQExchangeTypeEnum } from '../../../adapters/rabbitmq/interface';
import { RedisPropagatorService } from '../../../adapters/redis';
import { EXCHANGE, KEY, QUEUE } from './message.handler.enum';
 ...
 * Queue
 */
export const _QUEUE: string = `${config.get('APPLICATION_NAME')}_messages`;
...
const handler = async (
  ...
  try {
    ...
    if (logger)
      logger.error(`[MessageHandler] -> Error on handler (${error.message})`);
